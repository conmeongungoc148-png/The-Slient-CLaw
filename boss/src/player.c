...
static void UpdateBossPlayerInternal(BossPlayer *p, cute_tiled_map_t *map, float mapOffsetY, float fallbackGroundY, float dt) {
    ...
    if (p->isJumping) {
        ...
        float floorY = map ? MapGetFloorYAtX(map, p->position.x, currentY, p->position.y, mapOffsetY, fallbackGroundY) : fallbackGroundY;
        p->groundY = floorY;
        ...
        float floorY = map ? MapGetFloorYAtX(map, p->position.x, p->position.y, p->position.y + 12.0f, mapOffsetY, fallbackGroundY) : fallbackGroundY;
