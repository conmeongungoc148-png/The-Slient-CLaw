...
void InitBossPlayer(BossPlayer *p, Vector2 pos, float groundY) {
  ...
  p->hurtBox = (Rectangle){pos.x - 10.0f, pos.y - 14.625f, 20.0f, 14.625f};
  p->speed = 287.5f;
  p->groundY = groundY;
  p->hp = PLAYER_MAX_HP;
  p->facingRight = true;
...
static void UpdateBossPlayerInternal(BossPlayer *p, cute_tiled_map_t *map,
                                     float mapOffsetY, float fallbackGroundY,
                                     float dt) {
  UpdateCheatBuffer(p);
  ...
  if (!gPreIntroSlowWalk && IsMouseButtonPressed(MOUSE_LEFT_BUTTON) &&
      !p->isAttacking && !p->isHurt) {
    p->isAttacking = true;
    p->currentFrame = 0;
    p->frameTimer = 0.0f;
    p->freezeTimer = 0.15f;
  ...
  if (p->isJumping) {
    ...
    float floorY =
        map ? MapGetFloorYAtX(map, p->position.x, currentY, p->position.y,
                              mapOffsetY, fallbackGroundY)
            : p->groundY;
    p->groundY = floorY;
    ...
    float floorY =
        MapGetFloorYAtX(map, p->position.x, p->position.y,
                        p->position.y + 12.0f, mapOffsetY, fallbackGroundY);
    p->groundY = floorY;
  ...
  p->hurtBox.x = p->position.x - 10.0f;
  p->hurtBox.y = p->position.y - 14.625f;
}
...
void UpdateBossPlayerOnMap(BossPlayer *p, cute_tiled_map_t *map,
                           float mapOffsetY, float fallbackGroundY, float dt) {
  UpdateBossPlayerInternal(p, map, mapOffsetY, fallbackGroundY, dt);
}

void DrawBossPlayer(BossPlayer *p, Texture2D idle, Texture2D walk,
                    Texture2D run, Texture2D jump, Texture2D runJump,
                    Texture2D attack, Texture2D hurt) {
  if (p->godMode) {
    DrawText("GOD MODE", GetScreenWidth() - 180, 40, 20,
             (Color){255, 215, 0, 220});
  }
  if (gCheatNotifTimer > 0) {
    unsigned char a = (unsigned char)(gCheatNotifTimer / 3.0f * 255);
    DrawText(gCheatNotifText, GetScreenWidth() / 2 - 90,
             GetScreenHeight() / 2 + 100, 24, (Color){0, 255, 0, a});
  }
  ...
  Rectangle source = {(float)frameIdx * (PLAYER_FRAME_W + PLAYER_SPACING), 0,
                      (float)PLAYER_FRAME_W, (float)PLAYER_FRAME_H};
  if (p->facingRight)
    source.width = -source.width;

  float scale = 1.2f;
  // Dịch hình ảnh tiến lên 9px để hurtbox lùi về sau 9px
  float spriteOffsetX = p->facingRight ? 9.0f : -9.0f;
  Rectangle dest = {p->position.x + spriteOffsetX, p->position.y + 19.2f,
                    (float)PLAYER_FRAME_W * scale,
                    (float)PLAYER_FRAME_H * scale};
  Vector2 origin = {(float)PLAYER_FRAME_W * scale / 2.0f,
                    (float)PLAYER_FRAME_H * scale};
